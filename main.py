    ...
    try:
        if salary['from'] is not None and salary['currency'] == 'RUR':
            if salary['to'] is not None:
                zarplata = (salary['from'] + salary['to']) / 2  # Зарплата от нормальных значений
                return zarplata
            elif salary['to'] is None:
                zarplata = salary['from'] * 1.2  # Зарплата от значения "от"
                return zarplata
            else:
                return None
        elif salary['from'] is None and salary['to'] is not None:
            zarplata = salary['to'] * 0.8  # Зарплата до значения "до"
            return zarplata
        else:
            return None
    except:
